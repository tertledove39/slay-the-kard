...
public partial class BattleField : Control
{
    ...
    public void AddCardToPlace(string id, string place)
    {
        cardControl card;
        card = LoadCard(id);
        AddChild(card);
    ...
    async public Task EnemyTurn()
    {
        ...
        player2.AddPointMaxNatural();
        player2.DrawCard(1);
        foreach (var _card in player2.myCards)
        {
            if (_card.state == CardState.placed)
            {
                _card.GetDefence(2);
            }
            if (_card.state == CardState.inHand && _card.cardType != CardTypes.Command && _card.isFriend == 1 && _card.isHq == HQ.normalCard)
            {
                if (player2.point > _card.cost)
                {
                    var minPlaceName = "";
                    var allPlacesDictionary = getAllPlacesDictionary;
                    foreach (var place in allPlacesDictionary)
                    {
                        if (place.Value.Name == "1" || place.Value.Name == "2" ||place.Value.Name == "4" || place.Value.Name == "5")
                        {
                            minPlaceName = place.Value.Name;


                            if (allPlacesDictionary[minPlaceName].myCard != null)
                            {
                                minPlaceName = place.Value.Name;
                                continue;
                            }
                            allPlacesDictionary[minPlaceName].myCard = _card;
                            _card.myPlace = allPlacesDictionary[minPlaceName];
                            _card.targetPosition = allPlacesDictionary[minPlaceName].GlobalPosition + new Vector2(90, 110);
                            _card.state = CardState.placed;
                            if (_card.inHand != null)
                            {
                                _card.inHand.RemoveCard(_card);
                                _card.inHand = null;
                            }


                            _card.myPlayer.point -= _card.cost;
                            _card.battleField.Refresh();
                            _card.battleField.cardInPlaces.Add(_card);
                        }
                    }
