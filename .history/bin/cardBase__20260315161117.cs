...
/// <returns></returns>
    ...
    private void SetupHoverHighlight()
    {
        ...
        if (hoverHighlight == null)
        {
            ...
            hoverHighlight.AnchorLeft = 0f;
            hoverHighlight.AnchorTop = 0f;
            hoverHighlight.AnchorRight = 1f;
            hoverHighlight.AnchorBottom = 1f;
            hoverHighlight.MarginLeft = 0f;
            hoverHighlight.MarginTop = 0f;
            hoverHighlight.MarginRight = 0f;
            hoverHighlight.MarginBottom = 0f;
            hoverHighlight.SizeFlagsHorizontal = (int)Control.SizeFlags.Fill;
            hoverHighlight.SizeFlagsVertical = (int)Control.SizeFlags.Fill;
            hoverHighlight.ZIndex = 30;

            var style = new StyleBoxFlat();
            style.BorderColor = new Color(1f, 0.85f, 0.2f, 0.95f);
            style.BorderWidth = new Vector2(4, 4);
    ...
    public void SetHover(bool hover, float targetScale = 1f)
    {
        // 无需重复设置（避免创建过多 Tween）
        if (isHovering == hover && Mathf.IsEqualApprox(Scale.x, hover ? targetScale : originalScale.x) && Mathf.IsEqualApprox(Scale.y, hover ? targetScale : originalScale.y))
            return;
        ...
        hoverTween.TweenProperty(this, "scale", wantedScale, 0.15f);

        // 亮度边框（通过 alpha 逐渐显示/隐藏）
        hoverHighlight.Visible = true;
        float targetAlpha = hover ? 1f : 0f;
        hoverTween.TweenProperty(hoverHighlight, "modulate:a", targetAlpha, 0.15f);
