...
/// </summary> 
    ...
    async public Task DiscardCard()
    {
        ...
        // 计算画面边缘的目标位置（右边边缘，垂直位置保持不变）
        Vector2 edgePosition = new Vector2(viewportSize.X + 100, Position.Y);
        
        // 步骤1: 移动到画面边缘 (0.5秒)
        var moveTween = CreateTween();
        moveTween.SetTrans(Tween.TransitionType.Sine);
        moveTween.SetEase(Tween.EaseType.InOut);
        moveTween.TweenProperty(this, "position", edgePosition, 0.5f);
        ...
        rotateTween.TweenProperty(this, "rotation", Rotation + Mathf.Tau, 3.0f); // 旋转一整圈
        await ToSignal(rotateTween, Tween.SignalName.Finished);
        
        // 步骤3: 旋转着移出画面 (1秒)
        var exitTween = CreateTween();
        exitTween.SetTrans(Tween.TransitionType.Sine);
        exitTween.SetEase(Tween.EaseType.In);
        exitTween.SetParallel(true); // 并行执行旋转和移动
        exitTween.TweenProperty(this, "position", new Vector2(viewportSize.X + 500, Position.Y), 1.0f);
        exitTween.TweenProperty(this, "rotation", Rotation + Mathf.Tau, 1.0f);
