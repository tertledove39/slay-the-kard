...
/// </summary>
    ...
    /// <summary>
    /// 计算表达式（支持 + - * / 和括号），并向下取整
    /// 支持变量：&result、&targets.count
    /// </summary>
    private int EvaluateExpression(string expr, int result, List<cardBase_> targets = null)
    {
        if (string.IsNullOrWhiteSpace(expr))
            return 0;

        expr = expr.Trim();
        expr = expr.Replace("&result", result.ToString());
        if (targets != null)
        {
            expr = expr.Replace("&targets.count", targets.Count.ToString());
        }
        ...
        foreach (var segment in effectSegments)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                else if (instruction.StartsWith("GetTargetByIndex"))
                {
                    ...
                    if (match.Success)
                    {
                        int index = EvaluateExpression(match.Groups[1].Value, result, targets);
                ...
                if (instruction.StartsWith("Heal", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (match.Success)
                    {
                        int healAmount = EvaluateExpression(match.Groups[1].Value, result, targets);
                ...
                if (instruction.StartsWith("GetAttack"))
                {
                    ...
                    if (match.Success)
                    {
                        int GetAttackAmount = EvaluateExpression(match.Groups[1].Value, result, targets);
                        foreach (var target in targets)
                        {
                            if (target != null)
                            {
                ...
                if (instruction.StartsWith("SetDefence"))
                {
                    ...
                    if (match.Success)
                    {
                        int setDefenceAmount = EvaluateExpression(match.Groups[1].Value, result, targets);
                        foreach (var target in targets)
                        {
                            if (target != null)
                            {
                ...
                if (instruction.StartsWith("damage"))
                {
                    ...
                    if (match.Success)
                    {
                        int damageAmount = EvaluateExpression(match.Groups[1].Value, result, targets);
                        foreach (var target in targets)
                        {
                            if (target != null)
                            {
                ...
                if (instruction.StartsWith("DrawACard"))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        int count = EvaluateExpression(match.Groups[2].Value, result, targets);
                ...
                if (instruction.StartsWith("DrawUnitCards"))
                {
                    ...
                    if (match.Success)
                    {
                        int count = EvaluateExpression(match.Groups[1].Value, result, targets);
                        if (sourceCard?.GetIsFriend() == IsFriend.friend)
                        {
                ...
                if (instruction.StartsWith("setResult"))
                {
                    ...
                    if (match.Success)
                    {
                        int value = EvaluateExpression(match.Groups[1].Value, result, targets);
                        result = value;
                    }
                }
                ...
                if (instruction.StartsWith("GetRandomNumber"))
                {
                    ...
                    if (match.Success)
                    {
                        int min = EvaluateExpression(match.Groups[1].Value, result, targets);
                        int max = EvaluateExpression(match.Groups[2].Value, result, targets);
                        var rnd = new Random();
