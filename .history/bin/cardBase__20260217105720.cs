...
/// <param name="n"></param>
    public void GetDefence(int n)
    {
        ...
        // 触发闪烁和颜色变化效果
        FlashAttributeWithColor("defence", defence);
        RefreshState();
    }
...
/// <param name="n"></param>
    public void GetAttack(int n)
    {
        ...
        // 触发闪烁和颜色变化效果
        FlashAttributeWithColor("attack", attack);
        RefreshState();
    }
...
/// </summary> 
    ...
    /// 如果等于初始值：变为黑色
    /// </summary>
    /// <param name="attributeName">属性名："attack" 或 "defence"</param>
    /// <param name="currentValue">当前属性值</param>
    private void FlashAttributeWithColor(string attributeName, int currentValue)
    {
        if (!IsNodeReady())
            return;

        Label targetLabel = GetNode<Label>(attributeName);
        if (targetLabel == null)
            return;

        // 停止之前的动画
        if (currentFlashTween != null && currentFlashTween.IsValid())
        {
            currentFlashTween.Kill();
        }

        // 确定初始值和历史最大值
        int initialValue = (attributeName == "attack") ? initialAttack : initialDefence;
        int maxValue = (attributeName == "attack") ? maxHistoryAttack : maxHistoryDefence;

        // 根据当前值和历史最大值确定颜色
        Color targetColor;
        if (currentValue == initialValue)
        {
            targetColor = Colors.Black;  // 初始值 - 黑色
        }
        else if (currentValue >= maxValue)
        {
            targetColor = Colors.Green;  // 达到或超过历史最大值 - 绿色
        }
        else
        {
            targetColor = Colors.Red;    // 小于历史最大值 - 红色
        }

        // 创建闪烁动画
        currentFlashTween = CreateTween();
        currentFlashTween.SetLoops(3);  // 闪烁3次
        currentFlashTween.TweenCallback(() => targetLabel.AddThemeColorOverride("font_color", targetColor));
        currentFlashTween.TweenCallback(() => targetLabel.AddThemeColorOverride("font_color", Colors.Black));
        currentFlashTween.SetTrans(Tween.TransitionType.Linear);
        currentFlashTween.SetParallel(false);
        currentFlashTween.TweenInterval(0.15f);  // 每次闪烁间隔150ms
