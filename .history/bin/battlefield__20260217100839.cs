...
/// </summary>
    ...
    private async Task TriggerUnitEffects(string triggerPoint, cardBase_ sourceCard, List<cardBase_> targetCards = null)
    {
        if (targetCards == null) targetCards = new List<cardBase_>();

        // 检查场上所有单位
        var allUnits = ReadCardInPlaces().Where(x => x.getState() == CardState.placed).ToList();
        ...
        foreach (var unit in allUnits)
        {
            ...
            // 触发此单位的效果（unit 作为 this，sourceCard 作为触发者，targetCards 作为 target）
            await ParseAndExecuteEffect(actualEffect, unit, targetCards, sourceCard);
        }
    }
    ...
    public async Task Attack(cardBase_ from,cardBase_ to)
    {
        ...
        // 触发攻击者的 Attacking 效果
        await TriggerUnitEffects("Attacking", from, new List<cardBase_> { to });
        
        // 触发被攻击者的 BeingAttacked 效果
        await TriggerUnitEffects("BeingAttacked", to, new List<cardBase_> { from });
    ...
    void Move(cardBase_ card, place_ position)
    {
        ...
        // 触发移动单位的 Moving 效果
        _ = TriggerUnitEffects("Moving", card, new List<cardBase_> { card });
    }
    ...
    private List<cardBase_> GetAllowedTargets(cardBase_ attacker)
    {
        ...
        {
            return ReadCardInPlaces().Where(x => x.getState() == CardState.placed && x.GetIsFriend() != attacker.GetIsFriend()).ToList();
        }
        ...
        foreach (var place in allowedPlaces)
        {
            ...
            if (c != null && c.getState() == CardState.placed && c.GetIsFriend() != attacker.GetIsFriend()) results.Add(c);
        }
