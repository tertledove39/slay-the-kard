...
/// </summary>
    public void RefreshMyHand()
    {
        ...
        float midIndex = (cardsInHand.Count - 1) / 2f;
        float baseX = initPos.X - midIndex * spacing;

        for (int i = 0; i < cardsInHand.Count; i++)
        {
            float selectedCenter = hoveredHandIndex >= 0 ? hoveredHandIndex : midIndex;
            float distFromCenter = i - selectedCenter;
            float norm = midIndex == 0 ? 0f : distFromCenter / midIndex; // -1..1

            // 弧线布局：中心卡片最高，两侧逐渐降低
            float arcOffset = -handArcHeight * (1f - Mathf.Abs(norm));
            ...
            float normalizedDist = Mathf.Clamp(norm, -1f, 1f);
            float tiltFactor = Mathf.Pow(Mathf.Abs(normalizedDist), 1.2f);
            float targetRotationDeg = Mathf.Sign(normalizedDist) * tiltFactor * maxTiltDeg;
            ...
            _ = cardsInHand[i].MoveToPosition(target, 0.2f);
        }
    }
