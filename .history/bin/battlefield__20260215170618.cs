...
/// </summary> 
    ...
    {
        var _cardInPlaces = SortCardList(cardInPlaces).AsEnumerable().Reverse().ToList().Where(card => card.getState()== CardState.placed).ToList();
        ...
        foreach (var card in _cardInPlaces)
        {
            if (card!= cardNowChoose) MoveChild(card,0);
            card.ZIndex = 10;
            ...
            if (mouseButton.Pressed==false)
            {
                ...
                if(cardNowChoose!= null)
                {
                    ...
                    //松开时检查是否在任意一个合法的格子上
                    {
                        ...
                        if(result != null && result.GetMyCard()== null)// && CheckIfThePlaceIsOccupied(result) == false)
                        {
                        ...
                        else
                        {
                            ...
                            else if (cardNowChoose.GetMyPlace() != null && cardNowChoose.getState() == CardState.inplaceAndCaught)
                            {
                                cardNowChoose.setState(CardState.placed);
                            }
...
/// </summary>
    ...
    {
        _ = player1.DrawCard();
        var cardRes = ResourceLoader.Load<PackedScene>("res://bin/cardbase.tscn");
        var card = cardRes.Instantiate() as cardBase_;
        card.SetCardInformation(cardMaganer.GetCard("moscow"));
        foreach(var __card in cardInPlaces.Where(card=>card.getState() == CardState.placed))
        {
            __card.SetIsFriend(IsFriend.enemy);
        }
        AddCardToPlace(card,GetPlaceById("Place13"));
