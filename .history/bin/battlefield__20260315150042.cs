...
/// </summary>
        ...
        while (i < effectString.Length)
        {
            ...
            if (effectString[i] == '&' && i + 1 < effectString.Length)
            {
                ...
                while (varEnd < effectString.Length &&
                       (char.IsLetterOrDigit(effectString[varEnd]) || effectString[varEnd] == '_' || effectString[varEnd] == '.'))
                {
                    varEnd++;
                }
                ...
                switch (varName)
                {
                    ...
                    case "source.attack":
                        if (sourceCard != null)
                            sb.Append(sourceCard.ReadAttack());
                        else
                            sb.Append(0);
                        break;
                    case "source.defence":
                        if (sourceCard != null)
                            sb.Append(sourceCard.ReadDefence());
                        else
                            sb.Append(0);
                        break;
                    case "source.cost":
                        if (sourceCard != null)
                            sb.Append(sourceCard.ReadCost());
                        else
                            sb.Append(0);
                        break;
                    default:
    ...
    /// <summary>
    /// 计算表达式（支持 + - * / 和括号），并向下取整
    /// 支持变量：&result、&targets.count、&source.attack、&source.defence、&source.cost
    /// </summary>
...
/// </summary>
    ...
    private bool EvaluateCondition(string condition, int result, List<cardBase_> targets, cardBase_ sourceCard)
    {
        ...
        // 先替换表达式中的变量（如 &result、&targets.count、&source.attack 等）
...
public class Player
{
    ...
    // 手牌布局/悬停参数（可根据需要调整）
    private float handSpacing = 120f;          // 卡牌之间的间距
    private float hoverRaise = 40f;            // 悬停时抬起高度
    private float hoverScale = 1.08f;          // 悬停时放大倍数
    private float hoverSideMaxOffset = 30f;    // 悬停时侧边卡牌最大让开距离
...
/// </summary>
    ...
    /// <summary>
    /// 将手牌排列成一排，并在鼠标悬停时把悬停的卡牌抬起，其他卡牌分散到两侧
    /// </summary>
    public void RefreshMyHand()
    {
        ...
        float midX = initPos.X;
        float left = midX - (cardsInHand.Count - 1) * handSpacing / 2;

        int leftCount = hoveredHandIndex >= 0 ? hoveredHandIndex : 0;
        int rightCount = hoveredHandIndex >= 0 ? cardsInHand.Count - 1 - hoveredHandIndex : 0;

        for (int i = 0; i < cardsInHand.Count; i++)
        {
            Vector2 target = new Vector2(left + i * handSpacing, initPos.Y);
            ...
            if (hoveredHandIndex >= 0)
            {
                ...
                {
                    float distanceFromHover = Math.Abs(i - hoveredHandIndex);
                    float factor;

                    if (i < hoveredHandIndex && leftCount > 0)
                        factor = 1f - (distanceFromHover / (float)leftCount);
                    else if (i > hoveredHandIndex && rightCount > 0)
                        factor = 1f - (distanceFromHover / (float)rightCount);
                    else
                        factor = 0f;

                    float shift = hoverSideMaxOffset * factor;
                    target.X += i < hoveredHandIndex ? -shift : shift;
                }
            }
            ...
            if (card != null)
            {
                ...
                _ = card.MoveToPosition(target, 0.2f);
            }
        }
    }
