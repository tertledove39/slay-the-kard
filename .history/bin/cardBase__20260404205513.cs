...
/// </summary>
    ...
    private void AdjustFontSizeToFit(Label label)
    {
        ...
        // 使用GetRect()获取Label的实际显示区域，而不是Size属性
        Rect2 rect = label.GetRect();
        ...
        if (maxSize <= 0) maxSize = 16; // 默认最大值

        int minSize = 8; // 最小字体大小

        int bestSize = FindBestFontSizeForLabel(label, font, text, minSize, maxSize, containerSize);
