...
/// </summary> 
    ...
    ///   - > 极值：红色
    /// </summary>
    /// <param name="attributeName">属性名："attack"、"defence" 或 "cost"</param>
    /// <param name="currentValue">当前属性值</param>
    /// <param name="initialValue">初始属性值</param>
    /// <param name="extremeValue">极值（最大值或最小值）</param>
    /// <param name="isInverted">是否反向判断（true表示越小越好，如价格）</param>
    private void FlashAttributeWithColor(string attributeName, int currentValue, int initialValue, int extremeValue, bool isInverted = false)
    {
        if (!IsNodeReady())
            return;
        ...
        // 先设置初始颜色为黑色
        targetLabel.AddThemeColorOverride("font_color", Colors.Black);
        ...
        for (int i = 0; i < 3; i++)
        {
            currentFlashTween.TweenCallback(Callable.From(() => targetLabel.AddThemeColorOverride("font_color", targetColor)));
            currentFlashTween.TweenInterval(0.15f);
            currentFlashTween.TweenCallback(Callable.From(() => targetLabel.AddThemeColorOverride("font_color", Colors.Black)));
            currentFlashTween.TweenInterval(0.15f);
        }
