...
/// </summary>
            ...
            if (effectString[i] == '&' && i + 1 < effectString.Length)
            {
                ...
                switch (varName)
                {
                    ...
                    case "场上的敌方单位数量":
                        ...
                        break;
                    case string s when (s.StartsWith("场上友方") || s.StartsWith("场上的友方")) && s.EndsWith("单位数量"):
                        {
                            int startIndex = s.IndexOf("友方") + 2;
                            int length = s.Length - startIndex - "单位数量".Length;
                            string typeName = length > 0 ? s.Substring(startIndex, length).Trim() : string.Empty;
                            var cardType = ParseCardTypeFromChinese(typeName);
                            sb.Append(cardType.HasValue ? GetFieldUnitCount(IsFriend.friend, cardType) : 0);
                        }
                        break;
                    case string s when (s.StartsWith("场上敌方") || s.StartsWith("场上的敌方")) && s.EndsWith("单位数量"):
                        {
                            int startIndex = s.IndexOf("敌方") + 2;
                            int length = s.Length - startIndex - "单位数量".Length;
                            string typeName = length > 0 ? s.Substring(startIndex, length).Trim() : string.Empty;
                            var cardType = ParseCardTypeFromChinese(typeName);
                            sb.Append(cardType.HasValue ? GetFieldUnitCount(IsFriend.enemy, cardType) : 0);
                        }
                        break;
                    case "友方总部防御力":
    ...
    private CardTypes? ParseCardTypeFromChinese(string typeName)
    {
        ...
        switch (typeName.Trim())
        {
            ...
            case "指挥卡":
                return CardTypes.Command;
            default:
                if (Enum.TryParse<CardTypes>(typeName, true, out var parsed))
                    return parsed;
                return null;
        }
    }
