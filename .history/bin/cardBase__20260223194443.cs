...
/// </summary>
    ...
    private void AdjustFontSizeToFit(Label label)
    {
        ...
        // 创建新的 LabelSettings 资源，为每个 Label 单独设置字体大小
        LabelSettings labelSettings = GD.Load<LabelSettings>("res://bin/labelSetting_1.tres").Duplicate() as LabelSettings;
    ...
    private void AdjustRichTextFontSizeToFit(RichTextLabel label)
    {
        ...
        Vector2 textSize = font.GetStringSize(plainText, fontSize:fontSize);

        // 如果文本超出容器，逐渐减小字体大小
        while ((textSize.Y > containerHeight) && fontSize > 8)
        {
            fontSize--;
