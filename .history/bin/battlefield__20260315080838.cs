...
/// </summary>
    ...
    {
        player1.AddCardToHand(cardMaganer.GetCard("雅克7"));
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                if (instruction == "End&")
                {
                    if (foreachStack.Count > 0)
                    {
                        ...
                        {
                            // 循环结束，弹出栈并恢复上一级循环的当前目标
                            foreachStack.Pop();
                            if (foreachStack.Count > 0)
                            {
                                var parent = foreachStack.Peek();
                                if (parent.savedTargets.Count > 0)
                                {
                                    targets = new List<cardBase_> { parent.savedTargets[parent.currentIndex] };
                                }
                                else
                                {
                                    targets = new List<cardBase_>();
                            ...
                            {
                                // 没有嵌套循环时恢复为最后一层保存的目标列表
                                targets = ctx.savedTargets;
                            }
                        }
                    }
