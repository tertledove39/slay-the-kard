...
/// </summary>
    public override void _Ready()
    {
        ...
        foreach (var section in configFile)
        {
            var card = new CardData();
            {
                ...
                // 读取单位特性
                if (configFile[section.Key].ContainsKey("traits"))
                {
                    string traitsString = configFile[section.Key]["traits"].ToString().Trim();
                    card.Traits = ParseTraits(traitsString);
                }
                else
                {
                    card.Traits = UnitTraits.None;
                }
            }
            _items[card.Id] = card;
        }
    ...
    /// <summary>
    /// 解析特性字符串，返回UnitTraits枚举
    /// </summary>
    /// <param name="traitsString">特性字符串，如 "Blitz|HeavyArmor"</param>
    /// <returns>UnitTraits枚举</returns>
    private UnitTraits ParseTraits(string traitsString)
    {
        ...
        string[] traitArray = traitsString.Split(new[] { '|', ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
        
        foreach (var trait in traitArray)
        {
            switch (trait.Trim().ToLower())
            {
                case "blitz":
                    result |= UnitTraits.Blitz;
                    break;
                case "determination":
                    result |= UnitTraits.Determination;
                    break;
                case "heavyarmor":
                    result |= UnitTraits.HeavyArmor;
                    break;
                case "smokescreen":
                    result |= UnitTraits.SmokeScreen;
                    break;
                case "guardian":
                    result |= UnitTraits.Guardian;
                    break;
                case "shock":
                    result |= UnitTraits.Shock;
                    break;
                case "ambush":
                    result |= UnitTraits.Ambush;
                    break;
                case "immunity":
                    result |= UnitTraits.Immunity;
                    break;
            }
        }
...
/// </summary>
    ...
    public async Task Attack(cardBase_ from,cardBase_ to)
    {
        ...
        // 执行反击
        if (canCounterAttack && willReceiveCounterAttack && to.ReadDefence() > 0)
        {
