...
/// </summary> 
    void RefreshAllCardDisplayOrder()
    {
        ...
        {
            foreach (var choiceCard in choiceCards)
            {
                if (choiceCard == null)
                    continue;

                if (cardInPlaces.Contains(choiceCard))
                {
                    MoveChild(choiceCard, 1);
                    choiceCard.ZIndex = 40;
                }
            }
        }
        ...
        {
            if (card != cardNowChoose && (choiceCards == null || !choiceCards.Contains(card)))
                MoveChild(card,1);
            card.ZIndex = 10;
        } 
