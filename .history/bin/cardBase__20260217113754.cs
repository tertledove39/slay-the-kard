...
/// </summary> 
    ...
    private void FlashAttributeWithColor(string attributeName, int currentValue, int initialValue, int extremeValue, bool isInverted = false)
    {
        ...
        {
            // 创建局部副本以避免闭包问题
            Color flashColor = targetColor;
            
            // 改变为目标颜色
            currentFlashTween.TweenCallback(Callable.From(() => {
                targetLabel.AddThemeColorOverride("font_color", flashColor);
            }));
        ...
        currentFlashTween.TweenCallback(Callable.From(() => {
            targetLabel.AddThemeColorOverride("font_color", finalColor);
        }));
    }
