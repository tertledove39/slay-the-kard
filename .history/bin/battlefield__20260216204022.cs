...
/// </summary>
                        ...
                        if(result != null)
                        {
                            if(result.GetMyCard()== null)
                            {
                                ...
                                else if(cardNowChoose.getState() == CardState.inplaceAndCaught)
                                {
                                    ...
                                    {
                                        // 从箭头起点到目标格中心绘制一次性箭头
                                        var fromPos = arrowFrom.GlobalPosition;
                                        var toPos = result.GetNode<Control>("Control").GlobalPosition;
                                        SpawnTempArrow(fromPos, toPos);
    ...
    // 在场上绘制一次性箭头（从 from 到 to），0.6s 后自动移除
    async void SpawnTempArrow(Vector2 from, Vector2 to)
    {
        var line = new Line2D();
        ...
        if (line != null && line.IsInsideTree()) line.QueueFree();
    }
