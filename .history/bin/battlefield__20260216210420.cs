...
/// </summary> 
    ...
    /// <summary>
    /// 敌方阵线
                        GD.Print($"Select Command card: {card.Name} state:{card.getState()}");
    /// </summary>
         ...
         if (@event is InputEventMouseButton mouseButton)
        {
            ...
            if (mouseButton.Pressed)
            {
                ...
                if (card != null && card.getState() == CardState.inHand)
                {
                    ...
                    {
                        cardNowChoose = card;
                        card.setState(CardState.commandCardCaught);
                        // 显示箭头起点
            ...
            if (mouseButton.Pressed==false)
            {
                ...
                if(cardNowChoose!= null )
                {
                    ...
                    //松开时检查是否在任意一个合法的格子上
                    {
                        ...
                        if(result != null)
                        {
                               GD.Print($"Release on place: {result.Name} targetHas:{result.GetMyCard() != null}");
                            if(result.GetMyCard()== null)
                            {
                                ...
                                else if(cardNowChoose.getState() == CardState.commandCardCaught)
                                {
                                    ...
                                    if (cardNowChoose.cardType == CardTypes.Command)
                                    {
                                        ...
                                        GD.Print($"Command card released on place: {result.Name} stateBefore:{cardNowChoose.getState()}");
                                    }
