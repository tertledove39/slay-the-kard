...
/// </summary> 
    ...
    {
        // 如果卡片在手中，通知 Player 更新悬停状态
        if (state == CardState.inHand && battleField != null && battleField.player1 != null)
        {
            var cardsInHand = battleField.player1.GetCardsInHand();
            int index = cardsInHand.IndexOf(this);
            if (index >= 0)
            {
                battleField.player1.SetHoveredHandIndex(index);
            }
        }
        else
        {
    ...
    {
        // 如果卡片在手中，通知 Player 取消悬停状态
        if (state == CardState.inHand && battleField != null && battleField.player1 != null)
        {
            battleField.player1.SetHoveredHandIndex(-1);
        }
        else
        {
            // 不在手中时，直接取消悬停
