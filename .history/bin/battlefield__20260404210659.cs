...
/// </summary>
    ...
    /// 如果是玩家单位，尝试将其返回手牌，若失败，将其弃置
    /// 如果是敌方单位，如果无法撤退到支援阵线，直接将其弃置
    /// 如果单位在支援阵线，尝试撤退到手牌，如果是敌方单位或无法撤退到手牌，就直接将其弃置
    /// </summary>
    ...
    private async Task RetreatUnit(cardBase_ unit)
    {
        ...
        bool isInFrontLine = frontLine.Contains(unitPlace) || enemySupprotLine.Contains(unitPlace);
        bool isInSupportLine = supportLine.Contains(unitPlace) || enemySupprotLine.Contains(unitPlace);
        bool isFriendly = unit.GetIsFriend() == IsFriend.friend;

        // 如果在前线，尝试移动到支援阵线
        if (isInFrontLine)
        {
            place_ supportPlace = null;
            if (isFriendly)
            {
                supportPlace = GetTheFirstValidFriendlyPlace();
            }
            else
            {
                supportPlace = GetTheFirstValidEnemySupportPlace();
            }
            ...
            if (supportPlace != null)
            {
                ...
                await AddCardToPlace(unit, supportPlace);
                return;
            }
        }
        ...
        {
            // 尝试返回手牌
            if (player1.GetCardsInHand().Count < player1.maxHandSize)
            {
                unitPlace.UnbondCard();
                RemoveCard(unit);
    ...
    /// 执行所有挂起的弃置操作
    /// </summary>
    private async Task ExecutePendingDiscards()
    {
        // 收集所有需要弃置的卡牌
        var cardsToDiscard = new List<cardBase_>();
        foreach (var card in cardInPlaces)
        {
            if (card != null && card.pendingDiscardTasks.Count > 0)
            {
        ...
        {
            await card.ExecutePendingDiscards();
        }
    }
