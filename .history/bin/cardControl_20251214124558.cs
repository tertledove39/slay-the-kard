...
public partial class cardControl : Control
{
    ...
    async public Task Dead()
    {
        state = CardState.destroyed;
        Visible = false;
        await ExecEffect(this,Times.dead);
        //if(isFriend == 0)
        //{
        //    battleField.SetTime(Times.aFriendlyUnitDead,[this]);
        //}
        //else
        //{
        //    battleField.SetTime(Times.aEnemyUnitDead,[this]);
        //}
        //battleField.ExecAllTargetsEffect();
    ...
    async public void RunEffect(cardControl target)
    {
        ...
        await ExecEffect(target,Times.played);
    ...
    public List<cardControl> TargetHandler(string input)
    {
        ...
        foreach(var section in sections){
            switch (section)
                    ...
                    if (section == "hq")
                    {
                        targets = battleField.cardInPlaces.FindAll(x => x.isHq == HQ.hq);
                    }
                    if (section == "unit")
                    {
                        targets = battleField.cardInPlaces.FindAll(x => x.isHq == HQ.normalCard);
                    }
                    }
        return targets;
    }
    public async Task ExecEffect(cardControl target,Times time)
    {
        foreach (var a_effect in effects)
        {
            if(time.ToString() != a_effect.Split(':')[0])
            {
                return;
            }
            List<cardControl> targets = [];
            ...
            foreach (var part in parts)
            {
                ...
                if (part.StartsWith("setTargets"))
                {
                    Match match = Regex.Match(part, @"\(([^)]*)\)");
                    if (match.Success)
                    {
                        var xStr = match.Groups[1].Value;
                        targets = TargetHandler(xStr);
                    }
                }
                if (part.StartsWith("setTarget"))
                {
                    targets = [target];
                }
                if (part.StartsWith("getCount"))
                {
                    Match match = Regex.Match(part, @"\(([^)]*)\)");
                    if (match.Success)
                    {
                ...
                if (part == "damage")
                {
                    foreach(cardControl _target in targets)
                    {
                        await _target.GetDefence(-result);
                    }
                }
                if (part == "heal")
                {
                ...
                if (part.StartsWith("Heal"))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        if (int.TryParse(xStr, out int x))
                        {
                            await myPlayer.myHQ.GetDefence(x);
                        }
                    }
                }
            }
    ...
    private void EffectInit()
    {
        if (effect != null)
        {
            if (effect.Contains('|'))
            {
                effects = effect.Split('|').ToList<string>();
            }
    ...
    public void OnButtonUp()
    {
        ...
        if (isFriend == 0 && battleField.playerTurn == 0)
        {
            ...
            //部署时点
            //await ExecEffect(this,Times.deployed);
            //battleField.SetTime(Times.aFriendlyUnitDeployed,[this]);
            //battleField.ExecAllTargetsEffect();
            //battleField.ClearTime();
    ...
    public override void _Process(double delta)
    {
        ...
        {
            Scale = new Vector2(((float)ZIndex+90) / (float)100, ((float)ZIndex+90) / (float)100);
