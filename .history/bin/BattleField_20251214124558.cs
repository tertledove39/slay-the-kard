...
public partial class BattleField : Control
{
    ...
    /// 当前的时点
    /// </summary>
    public Times nowTime = Times.none;
    /// <summary>
    /// 触发这个时点的对象的列表
    /// </summary>
    public List<cardControl> sources = [];
    ...
    /// 设置当前时点
    /// </summary>
    /// <param name="time">时点</param>
    /// <param name="sources_">来源单位</param>
    public void SetTime(Times time,List<cardControl> sources_)
    {
        nowTime = time;
        sources = sources_;
    }
    ...
    {
        nowTime = Times.none;
        sources = new List<cardControl>();
    }
    /// <summary>
    /// 根据目前的时点自动尝试执行所有卡的效果
    /// </summary>
    public async Task ExecAllTargetsEffect()
    {
        foreach (cardControl card in cardInPlaces)
        {
            await card.ExecEffect(card,nowTime);
        }
    }
    ...
    public void ExecAllTargetsEffect(Times time)
    {
        foreach (cardControl card in cardInPlaces)
        {
            //await card.ExecEffect(card,time);
        }
    }
...
/// <returns>返回一个位置</returns>
    ...
    async public Task EnemyUnitAttack(cardControl unit)
    {
    ...
    async public void EnemyUnitTryAttack(cardControl from)
    {
        ...
        if (targetLine == frontLine && from.attack >= myHQInstance.defence)
        {
            await Task.Delay(300);
            from.Attack(myHQInstance);
        }
        foreach (var place in targetLine)
        {
            if (place.myCard != null && place.myCard.isFriend == 0&&IsInstanceValid(place.myCard))
            {
                possibleTarget.Add(place.myCard);
            }
        }
        foreach (var target in possibleTarget.ToArray())
        {
            if (from.attack >= target.defence && IsInstanceValid(target) )
            {
                await Task.Delay(300);
                from.Attack(target);
                return;
    ...
    async public Task EnemyTurn()
    {
        ...
        await enemyHQInstance.GetDefence(2 + turn);
        ...
        if (turn == 3)
        {
            EnemyDeployUnit("t70");
            EnemyDeployUnit("t70");
            await enemyHQInstance.GetDefence(5);
        }
        if (turn == 5)
        {
            EnemyDeployUnit("i554");
            EnemyDeployUnit("i554");
            EnemyDeployUnit("i554");
            await enemyHQInstance.GetDefence(10);
        }
        if (turn == 7)
        {
            if (cardInPlaces.Count > 0)
            {
                foreach (var _card in cardInPlaces.ToList())
            {
                if (_card.isFriend == 0)
                {
                    await _card.GetDefence(-5);
                    await Task.Delay(300);
                }
            }
            }
            ...
            if (turn == 14)
            {
                foreach (var _card in cardInPlaces)
                {
                    await myHQInstance.GetDefence(-20);
                    await Task.Delay(300);
                }
            }
            await myHQInstance.GetDefence(-3);
    ...
    public override void _Ready()
    {
        ...
        var configFileDrawPile = new IniFile();
        configFileDrawPile.Load("bin\\DrawPile.ini");
        foreach(var _id in configFileDrawPile["debug"]["drawpile"].ToString().Split(','))
        {
            cardControl enemyInstance;
            enemyInstance = LoadCard(_id);
