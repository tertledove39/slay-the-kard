...
/// <returns></returns>
    async Task AddCardToPlace(cardBase_ card, place_ place)
    {
        ...
        // 触发部署效果
        await TriggerUnitEffects("deployed", card, new List<cardBase_>());
    }
...
/// </summary>
    ...
    public async Task Attack(cardBase_ from,cardBase_ to)
    {
        ...
        await TriggerUnitEffects("BeingAttacked", to, new List<cardBase_> { to, from });
    ...
    async Task Move(cardBase_ card, place_ position)
    {
        ...
        // 触发移动单位的 Moving 效果
        await TriggerUnitEffects("Moving", card, new List<cardBase_> { card });
    }
