            ...
            if (effectString[i] == '&' && i + 1 < effectString.Length)
            {
                ...
                switch (varName)
                {
                    ...
                    case "field.enemy.unit.count":
                        ...
                        break;
                    case string s when s.StartsWith("field.friend.unit.count."):
                        sb.Append(GetFieldUnitCount(IsFriend.friend, ParseCardTypeFromName(s.Substring("field.friend.unit.count.".Length))));
                        break;
                    case string s when s.StartsWith("field.enemy.unit.count."):
                        sb.Append(GetFieldUnitCount(IsFriend.enemy, ParseCardTypeFromName(s.Substring("field.enemy.unit.count.".Length))));
                        break;
                    ...
                    case "enemy.hq.defence":
                        ...
                        break;
                    case "friend.command.point":
                        sb.Append(player1?.ReadPoint() ?? 0);
                        break;
                    case "friend.command.point.max":
                        sb.Append(player1?.ReadPointMax() ?? 0);
                        break;
                    case "friend.hand.count":
                        sb.Append(player1?.GetCardsInHand().Count ?? 0);
                        break;
                    case "friend.deck.remaining.count":
