    ...
    public void SetHqImage(int number)
    {
        switch (number)
        {
            case 0: 
                ...
                break;
            case 1:
                GetNode<Sprite2D>("cardbase").Texture = GD.Load<Texture2D>("res://cards/HQ_berlin.png");
                break;
        }
    }
...
/// </summary>
    public void RefreshState()
    {
        ...
        else
        {
            ...
            GetNode<Sprite2D>("cardbase").Texture = GD.Load<Texture2D>(IconPath);
        }
        if                     (name!= null)
        {
    ...
    private void AdjustFontSizeToFit(Label label)
    {
        ...
        // 获取容器的尺寸
        float containerWidth = label.Size.X;
        float containerHeight = label.Size.Y;
        ...
        if (fontSize <= 0) fontSize = 12; // 默认字体大小
        ...
        // 计算文本尺寸
        Vector2 textSize = font.GetStringSize(text, (HorizontalAlignment)fontSize);

        // 如果文本超出容器，逐渐减小字体大小
        while ((textSize.X > containerWidth || textSize.Y > containerHeight) && fontSize > 8)
        {
            fontSize--;
            textSize = font.GetStringSize(text, (HorizontalAlignment)fontSize);
        }
    ...
    private void AdjustRichTextFontSizeToFit(RichTextLabel label)
    {
        ...
        if (fontSize <= 0) fontSize = 10; // 默认字体大小
        ...
        Vector2 textSize = font.GetStringSize(plainText, (HorizontalAlignment)fontSize);

        // 如果文本超出容器，逐渐减小字体大小
        while ((textSize.X > containerWidth || textSize.Y > containerHeight) && fontSize > 6)
        {
            fontSize--;
            textSize = font.GetStringSize(plainText, (HorizontalAlignment)fontSize);
        }
