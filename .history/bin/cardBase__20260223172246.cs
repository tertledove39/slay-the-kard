...
/// </summary>
    ...
    private void AdjustFontSizeToFit(Label label)
    {
        ...
        // 获取容器的尺寸
        float containerWidth = label.GetRect().Size.X;
        float containerHeight = label.GetRect().Size.Y;
        ...
        // 初始字体大小
        int fontSize = label.LabelSettings.FontSize;
    ...
    private void AdjustRichTextFontSizeToFit(RichTextLabel label)
    {
        ...
        // 优先尝试使用当前字体大小，如果文本超出容器，则逐渐减小字体大小
        bool fits = false;
        while (fontSize >= 6 && !fits)
        {
            // 计算文本尺寸
            Vector2 textSize = font.GetStringSize(plainText, HorizontalAlignment.Left, -1, fontSize);

            // 计算换行后的高度
            int lines = (int)Mathf.Ceil(textSize.X / containerWidth);
            float totalHeight = lines * textSize.Y;
