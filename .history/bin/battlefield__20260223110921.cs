    ...
    public async Task ParseAndExecuteEffect(string effectString, cardBase_ sourceCard, List<cardBase_> targetCards = null, cardBase_ targetCard = null)
    {
        ...
        foreach (var segment in effectSegments)
        {
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                {
                    var match = System.Text.RegularExpressions.Regex.Match(instruction, @"\(([A-Za-z]+),(\d+)\)");
                    if (match.Success)
                    {
...
/// <param name="number"></param>
    ...
    public async Task DrawCardsWithName(string namePattern, int count)
    {
        ...
        {
            var card = deck.FirstOrDefault(c => c.name.Contains(namePattern));
            if (card != null)
            {
                deck.Remove(card);
                if (cardsInHand.Count >= maxHandSize)
    ...
    {
        var cardsToDiscard = cardsInHand.Where(c => c.name.Contains(namePattern)).Take(count).ToList();
