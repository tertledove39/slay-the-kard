...
/// </summary> 
    ...
    ///   - > 极值：红色
    /// </summary>
    private async Task FlashAttributeWithColor(string attributeName, int currentValue, int initialValue, int extremeValue, bool isInverted = false)
    {
        // 根据属性名获取对应的Label节点
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            colorTween.TweenProperty(targetLabel, "modulate", targetColor, 0.5f);
            await Task.Delay(150);
            colorTween.TweenProperty(targetLabel, "modulate", Colors.Black, 0.5f);
            await Task.Delay(150);
        }
        Tween colorTween = CreateTween();
