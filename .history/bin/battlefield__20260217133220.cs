  ...
  /// <returns></returns>
    async Task AddCardToPlace(cardBase_ card, place_ place)
    {
        ...
        await card.MoveToPosition(place.GetPlaceGlobalPosition());
          ...
          // 触发部署效果
        await TriggerUnitEffects("BeingAddToPlace", card, new List<cardBase_>(), checkOnlySourceCard: true);
    }
...
TextureButton buttonNextTurn;
  ...
  /// </summary>
                            ...
                            else if(cardNowChoose.getState() == CardState.commandCardCaught)
                                {
                                    ...
                                    if (cardNowChoose.cardType == CardTypes.Command)
                                    {
                                        ...
                                        if (player1.UsePoint(cardNowChoose.ReadCost()))
                                        {
                                            ...
                                            {
                                                  // 目标合法,执行效果
                                                targetCard = result?.GetMyCard();
                                                          _          = ParseAndExecuteEffect(cardNowChoose.effect, cardNowChoose, targetCard != null ? new List<cardBase_> { targetCard } : null);
    ...
    async Task Move(cardBase_ card, place_ position)
    {
        ...
        await card.MoveToPosition(position.GetPlaceGlobalPosition());
        card.setState(CardState.placed);
        if(!frontLine.Contains(position))
        {
            await TriggerUnitEffects("Deployed", card, new List<cardBase_> { card }, checkOnlySourceCard: true);
            return;
        }
    ...
    async Task EnemyTurnAsync()
    {
        ...
        enemyHq.AddChange(ChangeType.GetDefence, 5);
        enemyHq.ExecChangeList();
        turn++;
        switch(turn)
        {
            case 1: 
